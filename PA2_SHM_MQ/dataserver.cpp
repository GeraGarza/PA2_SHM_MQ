...
int nchannels = 0;    //total channels
pthread_mutex_t newchannel_lock;
void *handle_process_loop(void *_channel);
char ival;
vector<string> all_data [NUM_PERSONS];    //list of channels

//make new channel
void process_newchannel_request (FIFORequestChannel *_channel)
{
    nchannels++;    //increase total size
    string new_channel_name = "data" + to_string(nchannels) + "_";    // name
    char buf [30];
    //strcpy - Copies the C string pointed by source into the array pointed by destination
    strcpy (buf, new_channel_name.c_str());
    //Cwrite- sends request to sever
    _channel->cwrite(buf, new_channel_name.size()+1);
    
    /*
     sets up a communication channel over “named pipe”
     first argument: a string that indicates the name of the channel
     the client has to create an instance with the same name
     */
    FIFORequestChannel *data_channel = new FIFORequestChannel(new_channel_name, RequestChannel::SERVER_SIDE, 256);
...
int main(int argc, char *argv[])
{
    ...
    FIFORequestChannel control_channel("control", RequestChannel::SERVER_SIDE,256);
